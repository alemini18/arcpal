...
__global__ void persistent_fixed_point_kernel(
    ...
    while (true) {
        ...
        // --- PHASE 1: PROPAGATE MODIFICATIONS (Grid-Stride Loop) ---
        ...
        // --- PHASE 2: EVALUATE DEDUCTIONS (Grid-Stride Block Loop) ---
        for (int rule_id = blockIdx.x; rule_id < num_rules; rule_id += gridDim.x) {
            if (*d_contradiction) break;
            if (d_touched_rules[rule_id] == 0) continue;
...
bool run_propagation_atom_oriented(PropagatorInput& input, ReverseTables& revt) {
    ...
    // Calculate maximum occupancy for the cooperative launch
    int num_blocks_per_sm;
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(&num_blocks_per_sm, persistent_fixed_point_kernel, 256, 0);
    ...
    int launch_blocks = std::min((input.num_rules + 255) / 256, max_blocks);
    if (launch_blocks == 0) launch_blocks = 1;
    ...
    cudaLaunchCooperativeKernel((void*)persistent_fixed_point_kernel, launch_blocks, 256, kernel_args);
    cudaDeviceSynchronize();
